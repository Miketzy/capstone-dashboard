...
import "./Vertebrates.css";
import axios from "axios";
import Pagination from "@mui/material/Pagination";
import { CiSearch } from "react-icons/ci";

function Vertebrates() {
  const [vertebrates, setVertibrates] = useState([]);
  ...
  useEffect(() => {
    ...
    axios
      .get("https://capstone-dashboard-d30v.onrender.com/getvertebrates")
      .then((res) => setVertibrates(res.data))
      .catch((err) => console.log(err));
  }, []);

  // Filter the data based on the search term
  const filteredVertebrates = vertebrates.filter((data) =>
    [
      data.specificname,
      data.scientificname,
      data.commonname,
  ...
  const currentItems = filteredVertebrates.slice(
    indexOfFirstItem,
    indexOfLastItem
  );
  ...
  return (
    <div className="vertebrates">
      ...
      <div className="vertebratestable-responsive">
        <table className="vertebratestable table-bordered">
          <thead className="vertebratesthead-dark7">
            <tr>
              <th>No.</th>
              <th>Specific Name</th>
